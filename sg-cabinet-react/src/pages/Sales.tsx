...
/* ===== Simple premium local: segmented buttons (page-level) ===== */
function SegBtn({
  active,
  onClick,
  children,
...
function toneByValue(v: number, a: number, b: number): HealthTone {
  // bad >= b, warn >= a else good
  if (v >= b) return 'bad';
  if (v >= a) return 'warn';
  return 'good';
}
...
export default function Sales() {
  ...
  const coinCents = Math.max(1, toInt(qAll.data?.settings?.coin_value_cents ?? settings.coin_value_cents ?? 100));
  ...
  const alerts = React.useMemo(() => {
    ...
    if (totals.pending >= 6) list.push({ key: 'pending', title: 'Зависшие подтверждения (критично)', sev: 'bad' });
    else if (totals.pending > 0) list.push({ key: 'pending', title: 'Есть зависшие подтверждения', sev: 'warn' });

    if (totals.cancelRate >= 0.12) list.push({ key: 'cancel', title: 'Высокий процент отмен', sev: 'bad' });
    else if (totals.cancelRate >= 0.08) list.push({ key: 'cancel', title: 'Процент отмен выше нормы', sev: 'warn' });
  ...
  const series = React.useMemo(() => {
    // chart data uses cents for money and raw counts for lines
    return days.map((d) => ({
      date: d.date,
  ...
  const topCashiers = [...cashiers].sort((a, b) => (b.revenue_cents || 0) - (a.revenue_cents || 0));
      ...
      aside={
        <div className="sgp-aside">
          <SgCard tone={healthTone === 'bad' ? 'bad' : healthTone === 'warn' ? 'warn' : 'good'}>
            ...
            <SgCardContent>
              <Tip tone={healthTone === 'bad' ? 'warn' : healthTone}>
                {healthTitle}
              </Tip>
              ...
              <Collapsible
                ...
                right={<span className="sgp-muted">{alerts.length ? `${alerts.length} алерт(а)` : 'нет алертов'}</span>}
              ...
              >
                {!alerts.length ? (
                  ...
                  alerts.slice(0, 6).map((a) => (
                    <div key={a.key} className={'sgpRow ' + (a.sev === 'bad' ? 'is-bad' : 'is-warn')}>
                      <div className="sgpRowLeft">
                        <div>
          ...
          <SgCard tone="neutral">
            <SgCardHeader
              right={<SgStatusBadge tone="dev" label="DEV" title="пока UI-only" />}
            >
              <div>
            ...
            <SgCardContent>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 10 }}>
                ...
                <div>
                  <div className="sgp-muted" style={{ marginBottom: 6 }}>Валюта</div>
                  <SgSelect
                    value={currencyDraft}
                ...
                <div className="sgp-muted">DEV: позже сделаем PUT /settings и invalidate queries</div>
              </div>
            </SgCardContent>
          </SgCard>
        </div>
    ...
    >
      ...
      <div className="sgp-salesKpiRow">
        <SgCard tone="neutral" variant="subtle">
          <SgCardHeader right={<IconBtn title="info" active={false} onClick={() => {}}>?</IconBtn>}>
            <div>
              ...
              <div className="sgp-salesKpiVal">{isLoading ? '—' : moneyFromCent(totals.revenueC, currency)}</div>
              <div className="sgp-salesKpiMeta">{isLoading ? ' ' : `в день: ${(totals.revenueC / Math.max(1, days.length) / 100).toFixed(2)} ${currencyLabel(currency)}`}</div>
            </div>
          </SgCardHeader>
        </SgCard>
        ...
        <SgCard tone="neutral" variant="subtle">
          <SgCardHeader right={<IconBtn title="info" active={false} onClick={() => {}}>?</IconBtn>}>
            <div>
              ...
              <div className="sgp-salesKpiMeta">{isLoading ? ' ' : `ср. чек: ${moneyFromCent(totals.avgCheck, currency)}`}</div>
            </div>
          </SgCardHeader>
        </SgCard>
        ...
        <SgCard tone="neutral" variant="subtle">
          <SgCardHeader right={<IconBtn title="info" active={false} onClick={() => {}}>?</IconBtn>}>
            <div>
              ...
              <div className="sgp-salesKpiMeta">{isLoading ? ' ' : `repeat: ${fmtPct(totals.repeat)}`}</div>
            </div>
          </SgCardHeader>
        </SgCard>
        ...
        <SgCard tone="neutral" variant="subtle">
          <SgCardHeader right={<IconBtn title="info" active={false} onClick={() => {}}>?</IconBtn>}>
            <div>
              ...
              <div className="sgp-salesKpiVal">{isLoading ? '—' : `${totals.cashbackCoins.toLocaleString('ru-RU')} мон`}</div>
              <div className="sgp-salesKpiMeta">{isLoading ? ' ' : `≈ ${moneyFromCent(totals.cashbackCent, currency)}`}</div>
            </div>
          </SgCardHeader>
        </SgCard>
        ...
        >
          <SgCardHeader
            right={
              <SgStatusBadge
                tone={totals.net < 0 ? 'warn' : 'ok'}
                label={totals.net < 0 ? 'WARN' : 'OK'}
              />
            }
          >
            <div>
              <SgCardSub>NET</SgCardSub>
              <div className="sgp-salesKpiVal">{isLoading ? '—' : moneyFromCent(totals.net, currency)}</div>
              <div className="sgp-salesKpiMeta">{isLoading ? ' ' : `списано: ${moneyFromCent(totals.redeemCent, currency)}`}</div>
            </div>
          </SgCardHeader>
        </SgCard>
      </div>
      ...
      <SgCard>
        <SgCardHeader
          right={
            <div className="sgpIconGroup">
              <IconBtn title="Net bars" active={showNet} onClick={() => setShowNet((v) => !v)}>P</IconBtn>
              <IconBtn title="Revenue area" active={showRevenue} onClick={() => setShowRevenue((v) => !v)}>R</IconBtn>
              <IconBtn title="Orders line" active={showOrdersLine} onClick={() => setShowOrdersLine((v) => !v)}>O</IconBtn>
            </div>
          }
        >
          <div>
        ...
        <SgCardContent>
          ...
          <div style={{ marginTop: 12 }} className="sgp-seg">
            ...
            <SegBtn active={tab === 'funnel'} onClick={() => setTab('funnel')}>Воронка</SegBtn>
            <SegBtn active={tab === 'cashiers'} onClick={() => setTab('cashiers')}>Кассиры</SegBtn>
            <SegBtn active={tab === 'customers'} onClick={() => setTab('customers')}>Клиенты</SegBtn>
            <SegBtn active={tab === 'live'} onClick={() => setTab('live')}>Live</SegBtn>
          </div>
      ...
      {tab === 'summary' ? (
        <div className="sgp-salesTwoCol">
          <SgCard
            tone={totals.pending >= 6 ? 'bad' : totals.pending > 0 ? 'warn' : 'good'}
            variant="subtle"
          >
            <SgCardHeader
              right={
          ...
          >
            ...
            <SgCardContent>
              ...
              <div className="sgp-muted">
                {isLoading
                  ? ' '
                  : totals.cancelRate >= 0.12
                    ? 'плохо'
                    : totals.cancelRate >= 0.08
                      ? 'риск'
                      : 'ок'}
              </div>
            </SgCardContent>
                        ...
                        <div className="sgpRowLeft">
                          <div>
                            ...
                            <div className="sgpRowMeta">
                              confirm {fmtPct(c.confirm_rate)} · cancel {fmtPct(c.cancel_rate)} · median {c.median_confirm_minutes.toFixed(1)}m
                            </div>
                          </div>
                        </div>
                        <div className="sgpRowRight">
            ...
            {isLoading ? (
              ...
              <div className="sgp-salesFunnel">
                <div className="sgpRow">
                  ...
                  <div className="sgpRowRight">
                    <div className="sgpRowVal">{funnel?.scanned} → {funnel?.recorded}</div>
                ...
                <div className="sgpRow">
                  ...
                  <div className="sgpRowRight">
                    <div className="sgpRowVal">{funnel?.pin_issued} → {funnel?.pin_used}</div>
                ...
                <div className={'sgpRow ' + (((funnel?.median_confirm_minutes ?? 0) > 5) ? 'is-warn' : 'is-good')}>
                  <div className="sgpRowLeft">
                    <div>
                  ...
                  <div className="sgpRowRight">
                    ...
                    <div className="sgpRowSub">{(funnel?.median_confirm_minutes ?? 0) > 5 ? 'медленно' : 'ок'}</div>
                  </div>
                </div>
          ...
          <SgCardContent>
            {isLoading ? (
              ...
              topCashiers.map((c) => {
                ...
                return (
                  <div key={c.cashier_label} className={'sgpRow ' + (tone === 'bad' ? 'is-bad' : tone === 'warn' ? 'is-warn' : 'is-good')}>
                    <div className="sgpRowLeft">
                      <div>
                        <div className="sgpRowTitle">{c.cashier_label}</div>
                        <div className="sgpRowMeta">
                          выручка: {moneyFromCent(c.revenue_cents, currency)} · заказы: {c.orders} · confirm: {fmtPct(c.confirm_rate)} · cancel: {fmtPct(c.cancel_rate)}
                        </div>
                  ...
                  <div key={c.customer_label} className={'sgpRow ' + (tone === 'warn' ? 'is-warn' : 'is-good')}>
                    <div className="sgpRowLeft">
                      <div>
                        <div className="sgpRowTitle">{c.customer_label}</div>
                        <div className="sgpRowMeta">
                          LTV: {moneyFromCent(c.ltv_cents, currency)} · заказов: {c.orders} · last: {c.last_seen} · сегмент: {c.segment}
                        </div>
                ...
                <div className="sgpRow is-good">
                  <div className="sgpRowLeft">
                    <div>
                      ...
                      <div className="sgpRowMeta">Покупка 520 ₽ · cashback +31 мон · кассир #2 · 12:44</div>
                    </div>
                  </div>
                  <div className="sgpRowRight">
                    <div className="sgpRowVal">ok</div>
                ...
                <div className="sgpRow is-good">
                  <div className="sgpRowLeft">
                    <div>
                      ...
                      <div className="sgpRowMeta">Списано 120 мон · net +12 ₽ · кассир #1 · 12:40</div>
                    </div>
                  </div>
                  <div className="sgpRowRight">
                ...
                <div className="sgpRow is-warn">
                  <div className="sgpRowLeft">
                    <div>
                      ...
                      <div className="sgpRowMeta">Ждёт подтверждения · кассир #2 · 12:33</div>
                    </div>
                  </div>
                  <div className="sgpRowRight">
...
/* Minimal CSS hooks (keep in sg-premium.scss if you want):
.sgp-salesKpiRow{ display:grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: 12px; }
@media (max-width:1100px){ .sgp-salesKpiRow{ grid-template-columns: repeat(2, minmax(0, 1fr)); } }
.sgp-salesKpiVal{ font-size: 22px; font-weight: 950; margin-top: 6px; }
.sgp-salesKpiMeta{ margin-top: 6px; font-size: 12px; opacity: .72; }
.sgp-salesTwoCol{ display:grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-top: 12px; }
@media (max-width:1100px){ .sgp-salesTwoCol{ grid-template-columns: 1fr; } }
.sgp-salesBig{ font-size: 28px; font-weight: 950; }
*/
