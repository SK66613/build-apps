export async function mount(root, props, ctx){
  ...
  $('.cal-monthbar .prev').addEventListener('click', ()=>{ current.setMonth(current.getMonth()-1); renderMonth(); loadSlots(); });
  $('.cal-monthbar .next').addEventListener('click', ()=>{ current.setMonth(current.getMonth()+1); renderMonth(); loadSlots(); });
