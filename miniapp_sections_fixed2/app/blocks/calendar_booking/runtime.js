export async function mount(root, props, ctx){
  ...
  if (userDurTitle){ durTitleEl.textContent = userDurTitle; durTitleEl.style.display=''; }
  else { durTitleEl.textContent=''; durTitleEl.style.display='none'; }

  const userSlotsTitle = (typeof props.slots_title === 'string') ? props.slots_title.trim() : 'Свободные слоты';
  if (userSlotsTitle){ slotsTitleEl.textContent = userSlotsTitle; slotsTitleEl.style.display=''; }
  else { slotsTitleEl.textContent=''; slotsTitleEl.style.display='none'; }
  ...
  root.addEventListener('click', async (e)=>{
    ...
    if (e.target.closest('.cal-book')){
      const hh = root.querySelector('.slot.is-active')?.dataset.hh; if (!hh || isPreview) return;
      const r = await window.api('calendar.book', { date: selectedISO, time: hh, duration_min: activeDuration, contact: '', format: 'tg_call' });
